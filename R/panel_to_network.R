sanity_check_pre = function(unit_time, dyad_time) {
    ...
    units_setdiff = sort(unique(c(
        setdiff(units_dyad, units_unit),
    ...
    times = unique(times)
    times_setdiff = sort(unique(c(
        setdiff(times_dyad, times_unit),
...
dependent_network = function(network_dependent, network_environment, ...) {
    ...
    tmp = lapply(seq_along(network_environment$vertex.attributes), function(i) {
        network::network(
            x = network_environment[[network_dependent]][[i]],
